...
public class TileWindow : EditorWindow
{
    ...
    public class SceneViewEventHandler : Editor
    {
                ...
                if (isDraw)
                {
                    ...
                    if (selected != DRAWOPTION.select)
                    {
                        ...
                        if ((e.type == EventType.MouseDrag || e.type == EventType.MouseDown) && e.button == 0 && activeSprite != null)
                        {
                            ...
                            GameObject[] allgo = GameObject.FindGameObjectsWithTag("Tile") as GameObject[];
                            int brk = 0;
                            ...
                            else if (selected == DRAWOPTION.erase)
                            {
                                for (int i = 0; i < allgo.Length; i++)
                                {
                                    if (Mathf.Approximately(allgo[i].transform.position.x, mouseWorldPos.x) && Mathf.Approximately(allgo[i].transform.position.y, mouseWorldPos.y) && Mathf.Approximately(allgo[i].transform.position.z, mouseWorldPos.z))
                                    {
                                        UpdateNearbyTiles(mouseWorldPos, allgo, allgo[i], false);
        ...
        private static void UpdateNearbyTiles(Vector3 mouseWorldPos, GameObject[] allgo, GameObject newgo, bool isAddition)
        {
            for (int i = 0; i < allgo.Length; i++)
            {
    ...
    void OnGUI()
    {
        ...
        if (!Directory.Exists(Application.dataPath + "/Resources/Tilemaps/"))
        {
            //Directory.CreateDirectory(Application.dataPath + "/Tilemaps/");
            AssetDatabase.CreateFolder("Assets/Resources", "Tilemaps");
            AssetDatabase.Refresh();
            Debug.Log("Created Tilemaps Directory");
        }
        files = Directory.GetFiles(Application.dataPath + "/Resources/Tilemaps/", "*.png");
        options = new string[files.Length];
        EditorGUILayout.LabelField("Tile Map", GUILayout.Width(256));
        for (int i = 0; i < files.Length; i++)
        {
            options[i] = files[i].Replace(Application.dataPath + "/Resources/Tilemaps/", "");
        }
        ...
        {
            allSprites = AssetDatabase.LoadAllAssetsAtPath("Assets/Resources/Tilemaps/" + options[index]).Select(x => x as Sprite).Where(x => x != null).ToArray();
